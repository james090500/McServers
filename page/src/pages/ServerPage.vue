...
export default {
    ...
    methods: {
        ...
        likeServer() {
            this.axios.post(`http://localhost:8787/v1/server/${this.$route.params.hash}/like`);
            this.liked = true;
        }
    },
    created() {
        this.axios.get(`http://localhost:8787/v1/server/${this.$route.params.hash}`).then((response) => {
